<?php 
 ...
 include "functions.php"; 
    ...
    if(isset($_POST['blog_id']) && empty($_POST['blog_id'])){
        $array_blog = array("blog_naslov"=>"{$blog_naslov}","blog_naslov_en"=>"{$blog_naslov_en}","blog_ime_autora"=>"{$blog_ime_prezime}","blog_vrijeme"=>"{$blog_datum_kreiranja}","blog_uvodni_tekst"=>"{$blog_uvodni_tekst}","blog_uvodni_tekst_en"=>"{$blog_uvodni_tekst_en}","blog_glavni_tekst"=>"","blog_glavni_tekst_en"=>"","blog_cover"=>"{$blog_cover_name}","blog_alt"=>"{$blog_alt}","blog_alt_en"=>"{$blog_alt_en}","blog_tag"=>"{$blog_tag}","blog_tag_en"=>"{$blog_tag_en}","blog_datum_unosa"=>"","blog_status"=>"neaktivan");
        $red = dodajRedTabele('blog',$array_blog);
        echo $red;
    }else if(isset($_POST['blog_id']) && !empty($_POST['blog_id'])){
        $array_blog = array("blog_naslov"=>"{$blog_naslov}","blog_naslov_en"=>"{$blog_naslov_en}","blog_ime_autora"=>"{$blog_ime_prezime}","blog_vrijeme"=>"{$blog_datum_kreiranja}","blog_uvodni_tekst"=>"{$blog_uvodni_tekst}","blog_uvodni_tekst_en"=>"{$blog_uvodni_tekst_en}","blog_glavni_tekst"=>"","blog_glavni_tekst_en"=>"","blog_cover"=>"{$blog_cover_name}","blog_alt"=>"{$blog_alt}","blog_alt_en"=>"{$blog_alt_en}","blog_tag"=>"{$blog_tag}","blog_tag_en"=>"{$blog_tag_en}");
